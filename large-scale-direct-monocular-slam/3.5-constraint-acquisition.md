...
**当新的一帧关键帧** $$\mathcal{K}_{i}$$ **添加地图之后，进行闭环检测，即收集可能是（形成）闭环的（候选）关键帧** $$\mathcal{K}_{j_{1}},...,\mathcal{K}_{j_{n}}$$ **。**
...
**为了防止插入错误的或是假的闭环约束条件，我们要进行_逆向相似度检测_，即：**
...
**当只有两个变换估计在统计上很相似，（相似度检测）公式(20)的里面误差足够小的情况下(足够小的定义？译者额外添加备注)，这样的闭环约束（边）才被加入到全局地图里。**
