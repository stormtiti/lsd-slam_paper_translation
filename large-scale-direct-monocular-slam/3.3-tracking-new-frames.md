**3.3 对新图像帧的跟踪：直接图像配准**~~**基于** ~~$$\mathfrak{se}(3)$$~~ **的直接法配准**~~ Tracking new Frames: Direct $$\mathfrak{se}(3)$$ Image Alignment
...
改动\_Labby: 从当前关键帧作为参考帧，通过最小化方差归一化光度误差来计算当前帧图像相对参考帧的三维姿态变换。
