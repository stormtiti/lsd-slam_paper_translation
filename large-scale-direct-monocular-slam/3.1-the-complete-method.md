...
– Once a keyframe is replaced as tracking reference – and hence its depth map will not be refined further – it is incorporated into the global map by the map optimization component.

**—地图优化（模块）： 一旦关键帧被替换成图像跟踪的参考帧—它的深度图就不会进一步优化（refine）—并由地图优化（模块）合并入全局地图，（成为半稠密深度图，译者额外添加备注）。**
