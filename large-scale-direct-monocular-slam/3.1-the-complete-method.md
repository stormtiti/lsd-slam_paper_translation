...
**— 图像跟踪（模块）： 连续跟踪从相机获取到的新“图像帧”（即：新的输入观测量，译者额外添加备注）。也就是说，（对连续输入的图像），用前一帧图像的姿态作为初始姿态，**~~估算出新的图像帧到当前关键帧的刚体变换~~**估算出当前关键帧到新的图像帧的刚体变换** $$\mathbf{\xi} \in \mathfrak{se}(3)$$ **。** (谢谢 张明明的指点~)
