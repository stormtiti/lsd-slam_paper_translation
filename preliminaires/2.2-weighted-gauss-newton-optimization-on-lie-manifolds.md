...
**假设残差是独立的随机变量，那么可以用二阶海森—逆矩阵来近似表示** $$\mathbf{\Sigma}_{\mathbf{\xi}}$$ **， 其中， 二阶海森—逆矩阵的求解就是每次通过前一次迭代** $$(\textbf{J}^{T}\textbf{W}\textbf{J})^{-1}$$ **（用一阶雅克比来近似）计算得到。 那么求解相机姿态** $$\mathbf{\xi}^{(n)}$$ **数学模型，就可以看做是公式（10）， 左乘误差项** $$\epsilon$$ **服从高斯分布** $$\mathcal{N}(0, \mathbf{\Sigma}_\mathbf{\xi})$$ **。**
