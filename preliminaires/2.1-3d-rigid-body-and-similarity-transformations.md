...
![](/assets/math_17.png) **和 **![](/assets/math_16.png)** 之间的转换关系（即：指数映射和对数映射），位姿**~~结合律运算符，序连运算子~~**_乘法_（concatenation operator）** $$ \circ : \mathfrak{sim(3)} \times \mathfrak{sim(3)} \rightarrow \mathfrak{sim(3)}$$
