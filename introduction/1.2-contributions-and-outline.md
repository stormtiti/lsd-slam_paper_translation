...
**我们提出了一种**~~大规模，大尺度的基于直接法的单目SLAM~~**_基于直接法的大范围单目SLAM算法_（LSD-SLAM，英文简称），该方法不仅仅可以在局部本地，对相机（帧间）运动进行跟踪，而且还可以构建全局一致性的大规模大尺度的场景地图（请参看示图1.和2.）。**
...
**\(1\) 提供了一个**~~大规模，大尺度，基于直接法的单目~~**_基于直接法的大范围单目_ SLAM系统的（软件）框架，特别是用了（该软件框架包含）一种新颖的具备环境尺度感知（scale-aware）的图像配准算法，来直接估计，关键帧之间的相似变换即：** $$\mathbf{\xi} \in \mathfrak{sim}(3)$$
